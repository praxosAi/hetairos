...
def create_notion_tools(notion_client: NotionIntegration) -> List:
    ...
    @tool
    async def list_notion_workspaces() -> ToolExecutionResponse:
        """
    ...
    async def list_databases(account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        Args:
            account: Optional Notion workspace identifier. If not specified and user has only one workspace, that workspace will be used.
        """
        logger.info("Listing Notion databases...")
        try:
            # Fetch all databases
            databases = await notion_client.list_databases(account=account)
    ...
    async def list_notion_pages(account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            pages = await notion_client.search_pages(query="", account=account)
    ...
    async def query_notion_database(database_id: str, filter: Dict = None, sorts: List[Dict] = None, account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            results = await notion_client.query_database(database_id, filter, sorts, account=account)
    ...
    async def get_all_workspace_entries(account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            results = await notion_client.get_all_workspace_entries(account=account)
            response = ToolExecutionResponse(status="success", result=json.dumps(results))
    ...
    async def search_notion_pages_by_keyword(query: str, account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            results = await notion_client.search_pages(query, account=account)
            response = ToolExecutionResponse(status="success", result=json.dumps(results))
    ...
    async def create_notion_page(
        ...
        Args:
            title: Page title
            content: List of Notion blocks (content)
        ...
        try:
            page = await notion_client.create_page(
                title=title, content=content, parent_page_id=parent_page_id, account=account
            )
    ...
    async def create_notion_database_entry(
        ...
        try:
            page = await notion_client.create_page(
                title=title, content=content, database_id=database_id, properties=properties, account=account
            )
            response = ToolExecutionResponse(status="success", result={"page_link": page.get('url')})
    ...
    async def create_notion_database(
        ...
        try:
            database = await notion_client.create_database(
                parent_page_id=parent_page_id, title=title, properties=properties, account=account
            )
    ...
    async def append_to_notion_page(page_id: str, content: List[Dict[str, Any]], account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        Args:
            page_id: Page ID
            content: List of Notion blocks to append
            account: Optional Notion workspace identifier. If not specified and user has only one workspace, that workspace will be used.
        """
        logger.info(f"Appending content to Notion page: {page_id}")
        try:
            await notion_client.append_block_children(block_id=page_id, children=content, account=account)
    ...
    async def update_notion_page_properties(page_id: str, properties: Dict[str, Any], account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            await notion_client.update_page_properties(page_id=page_id, properties=properties, account=account)
    ...
    async def get_notion_page_content(page_id: str, account: Optional[str] = None) -> ToolExecutionResponse:
        ...
        try:
            content = await notion_client.get_page_content(page_id, account=account)
