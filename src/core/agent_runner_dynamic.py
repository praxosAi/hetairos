...
class DynamicAgentRunner:
    def __init__(self,trace_id: str, has_media: bool = False, override_tools: Optional[List] = None):
    ...
    async def run(self, user_context: UserContext, input: Dict, source: str, metadata: Optional[Dict] = None) -> AgentFinalResponse:
        ...
        try:
            ...
            if all_forwarded:
                logger.info("All input messages are forwarded; verify the user before taking actions.")
            ...
            if self.override_tools is not None:
                tools = self.override_tools
                logger.info(f"Using {len(tools)} overridden tools for this run.")
            else:
