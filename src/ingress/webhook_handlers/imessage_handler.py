    ...
    if not integration_record:
        ...
        try:
            ...
            if integration_record and user:
                ...
                await imessage_client.send_message(phone_number, f"This number is not authorized to use this bot. Please register with myPraxos on https://app.mypraxos.com?auto-connect=true&provider=imessage&overlay=connectors&phone_number={phone_number.replace('+','%2B')}")
                return {"status": "ok"}
        except Exception as e:
