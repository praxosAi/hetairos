...
def create_drive_tools(gdrive_integration: GoogleDriveIntegration) -> List:
    ...
    async def read_file_from_drive(file_name: str) -> ToolExecutionResponse:
        """Reads the content of a file from user's Google Drive ({user_email}) by its name. Supports Google Docs, Sheets, Slides, and text files."""
        try:
            ...
            return ToolExecutionResponse(status="error", system_error=str(e), user_message=f"Could not read the file '{file_name}'. Make sure it exists and is a supported file type (Google Docs, Sheets, Slides, or text files).")
    ...
    return [search_google_drive_files, save_file_to_drive, create_text_file_in_drive, read_file_from_drive]